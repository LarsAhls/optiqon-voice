...
### 5 — real round trip: **reported PASS by Lars, evidence partial**

Lars: the Groq key is entered during onboarding, and dictating with the base profile works.

What is measured, from root `u1`'s own preference file (no key material read, no transcript):

    provider_preset_id = groq
...
That corroborates the configuration but does not by itself count a dictation. The `DUMP_STATE`
hook could not answer for `u1` at all: it read `StorageRoot.DEFAULT` unconditionally, so on a
signed-in process it described the old unclaimed data — one profile, no keys, no dictations —
and would have been read as a FAIL of a step that had in fact passed. The hook now takes the
root to read (`--es root u1`), defaulting to the process's own; the same change is what step 10
needs to ask the isolation question from one side about the other.

**Step 5 therefore stands as reported-but-not-counted** until that dump runs: `dictations` and
`dictation_words` under `u1` greater than zero. The device detached before it could be
installed. This is exactly the distinction §10 of the plan insists on — documenting an outcome
is not achieving it — so step 5 is not marked PASS in the result table yet.
